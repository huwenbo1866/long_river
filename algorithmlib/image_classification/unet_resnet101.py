...
@license: by-nc-sa
@file: unet_resnet101.py
@time: 2021/1/28 上午10:23
@ 神经网络出处：https://github.com/niecongchong/UNet_resnet101-keras/blob/master/model/unet_resnet101.py
"""
...
def unet_resnet_101(input_shape, classes):
    """

    Parameters
    ----------
    input_shape:tuple，输入数据为图片的形状，如(64, 64, 3)，表示图片大小为 64 * 64 ，
    classes：int，图片分类的类别数量，如图片一共有六类，则为 “6”

    Returns: keras.Model, 由keras构建的神经网络模型
    -------
    ...
    # 进一步可以提取 kernel_initializer, activation等
