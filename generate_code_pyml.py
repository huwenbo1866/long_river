...
def generator_code():
    data, target = load_data_by_pandas(filepath="/个性化/art_literacy_strategy_longriver_local.csv", index_col=False, header="infer", delimiter=",", labels="quality", )
    train_data, test_data, train_target, test_target = train_test_split(data, target, test_size=0.2, random_state=42)
    pipeline_model=Pipeline([("StandardScaler_3",StandardScaler(copy=False, with_mean=True, with_std=False, )),("RandomForestClassifier_4",RandomForestClassifier(n_estimators=200, criterion="gini", max_depth=10, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0, max_features="sqrt", max_leaf_nodes=None, min_impurity_decrease=0, bootstrap=True, class_weight=None, ccp_alpha=0, oob_score=False, warm_start=False, n_jobs=None, random_state=42, )),])
    ...
    try:
        ...
        print('skip metric accuracy_score:', e)
    joblib.dump(pipeline_model, "Workbench/pipeline_model.pkl")
